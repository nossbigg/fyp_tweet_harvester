...
/**
 * Generates sample configuration files for reference
 *
 * Created by Gibson on 8/29/2016.
 */
public class AppSampleJsonConfigGenerator {
  public final String appDir;

  public AppSampleJsonConfigGenerator(String appDir){
    this.appDir = appDir;
  }
  ...
  public void generateSampleWorkers() {
    ...
    for (AbstractHTMLWorkerModel abstractHTMLWorker : abstractHTMLWorkerList) {
      ...
      // write to file
      File file = new File(appDir + "/" + abstractHTMLWorker.workerName + ".json");
      try {
  ...
  public void readWorkers() {
    ...
    List<File> files = (List<File>) FileUtils.listFiles(
        new File(appDir), listFileMatcher.toArray(new String[0]), true
    );
